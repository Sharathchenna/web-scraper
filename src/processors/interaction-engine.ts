...
export interface InteractionConfig {
  ...
  backoffMs?: number; // Base time in ms for exponential backoff between retries
}
...
export class InteractionEngine {
  private visitedElements = new WeakSet<any>();
   ...
   */
  async clickInteractiveElements(
    page: Page, 
    baseUrl: string, 
    harvestUrls: (page: Page, baseUrl: string) => Promise<string[]>,
    looksLikeArticleUrl: (url: string, baseUrl: string) => boolean
   ...
   */
  private async handleLoadMoreButtons(
    page: Page,
    baseUrl: string,
    harvestUrls: (page: Page, baseUrl: string) => Promise<string[]>,
    looksLikeArticleUrl: (url: string, baseUrl: string) => boolean
      ...
      for (let attempt = 0; attempt < this.config.maxLoadMoreClicks; attempt++) {
        ...
        for (const selector of INTERACTION_SELECTORS.loadMore) {
          try {
            ...
            if (foundButton) break; // Found button in this selector, move to next iteration
          } catch (error) {
   ...
   */
  private async handleReadMoreButtons(
    page: Page,
    baseUrl: string,
    harvestUrls: (page: Page, baseUrl: string) => Promise<string[]>,
    looksLikeArticleUrl: (url: string, baseUrl: string) => boolean
