...
interface JavaScriptHeavinessCheck {
  isHeavy: boolean;
  indicators: string[];
  score: number;
}
...
export class LinkDiscoverer {
  ...
  private static readonly PLATFORM_PATTERNS: Record<string, SlugPattern[]> = {
    'medium.com': [
   ...
   */
  private async detectJavaScriptHeaviness(url: string): Promise<JavaScriptHeavinessCheck> {
    let score = 0;
   ...
   */
  private async extractLinksFromContent(content: string): Promise<string[]> {
          ...
          for (const item of items.slice(0, LinkDiscoverer.MAX_FEED_ITEMS)) {
            if (item.link) {
              ...
              if (item.pubDate) {
                ...
                if (pubDate < oneYearAgo) continue;
              }
              urls.add(item.link);
            }
          }
          ...
          for (const entry of urlEntries.slice(0, LinkDiscoverer.MAX_FEED_ITEMS)) {
            if (entry.loc) {
              ...
              if (entry.lastmod) {
                ...
                if (lastMod < oneYearAgo) continue;
              }
              urls.add(entry.loc);
            }
          }
   ...
   */
  private async performAdvancedInteractions(page: Page, baseUrl: string): Promise<InteractionResult> {
    const discoveredUrls: Set<string> = new Set();
    const interactionsPerformed: string[] = [];
   ...
   */
  private async sniffNetworkRequests(baseUrl: string): Promise<string[]> {
    const urls: Set<string> = new Set();
    ...
    try {
      ...
      for (const request of requests) {
        ...
        if (request.method !== 'GET' || request.response?.status !== 200) continue;
  ...
  private looksLikeArticleUrl(url: string): boolean {
    try {
      ...
      if (path.match(/\.(jpg|jpeg|png|gif|css|js|woff2?)$/i)) return false;
      if (path.match(/^\/api\/|^\/static\/|^\/assets\//)) return false;
   ...
   */
  private extractUrlsFromJson(data: unknown, urlFields: string[]): string[] {
    const urls: string[] = [];
   ...
   */
  private isArticleUrl(url: string, baseUrl: string): boolean {
    try {
   ...
   */
  private async extractUrlsFromPage(page: Page): Promise<string[]> {
    const discoveredUrls: string[] = [];
   ...
   */
  private async simulateUserBehavior(page: Page, directoryUrl: string): Promise<string[]> {
    const urls: Set<string> = new Set();
   ...
   */
  public async discoverLinks(directoryUrl: string): Promise<LinkDiscoveryResult> {
    let discoveredUrls: string[] = [];
    let currentLayer = 1;
    let interactionsPerformed: string[] = [];
