...
export class Database {
  ...
  async addCrawlJob(job: Omit<CrawlJob, 'id' | 'created_at' | 'updated_at'>): Promise<CrawlJob> {
    await this.db.read();
  ...
  async updateCrawlJob(
    id: string, 
    updates: Partial<Pick<CrawlJob, 'status' | 'retry_count' | 'error_message'>>
  ): Promise<boolean> {
    await this.db.read();
  ...
  async getPendingJobs(limit: number = 10): Promise<CrawlJob[]> {
    await this.db.read();
  ...
  async getJobsByStatus(status: CrawlJob['status']): Promise<CrawlJob[]> {
    await this.db.read();
    
    return this.db.data.crawl_jobs.filter(job => job.status === status);
  }

  async getJobById(id: string): Promise<CrawlJob | undefined> {
    await this.db.read();
    
    return this.db.data.crawl_jobs.find(job => job.id === id);
  }

  async getJobsByTeam(teamId: string): Promise<CrawlJob[]> {
    await this.db.read();
    
    return this.db.data.crawl_jobs.filter(job => job.team_id === teamId);
  }

  async clearCompletedJobs(olderThanDays: number = 7): Promise<number> {
    await this.db.read();
