...
export class KnowledgeImporter {
    ...
    async trySmartDiscovery(directoryDoc, options, startTime) {
        try {
            ...
            const allDocuments = [directoryDoc]; // Include the directory page
    ...
    async extractLinksFromContent(content, baseUrl) {
        const urls = [];
        ...
        const filteredUrls = urls.filter(url => {
            ...
            if (url.includes('#') || url.includes('?'))
                return false;
            // Remove asset files
            if (url.match(/\.(jpg|jpeg|png|gif|svg|css|js|ico|woff|woff2|ttf|eot|pdf|zip|mp4|mp3)$/i))
                return false;
            // Remove API endpoints and technical paths
            if (url.match(/\/(api|_next|static|assets|public|dist|build|node_modules)\//))
                return false;
            // Remove self-references
            if (url === baseUrl || url === baseDomain)
                return false;
            // Keep blog-like paths
            if (url.match(/\/(blog|article|post|news|content|guide|tutorial|docs?|documentation)\//i))
                return true;
    ...
    async discoverBlogContent(content, baseUrl) {
        const urls = [];
    ...
    async detectWorkingUrlPattern(testTitle, baseDomain) {
    ...
    applyUrlPattern(title, baseDomain, pattern) {
    ...
    generatePotentialUrls(title, baseDomain, limited = false) {
        const urls = [];
