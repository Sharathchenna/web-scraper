...
export class KnowledgeBaseSerializer {
    ...
    async serialize(documents, teamId, userId) {
        try {
            ...
            if (this.enableCleaning && this.contentCleaner) {
                ...
                const cleaningResult = await this.contentCleaner.cleanAndExportDocuments(documents, teamId, userId);
            ...
            await fs.promises.writeFile(finalPath, JSON.stringify(knowledgeBase, null, 2), 'utf-8');
            // Generate summary
     ...
     */
    fallbackSerialization(documents, userId) {
     ...
     */
    mapContentType(sourceType, sourceUrl) {
        ...
        if (sourceUrl) {
            ...
            if (url.includes('podcast') || url.includes('transcript'))
                return 'podcast_transcript';
            if (url.includes('call') || url.includes('meeting'))
    ...
    generateSummary(knowledgeBase, originalDocuments) {
        ...
        const uniqueAuthors = new Set(knowledgeBase.items
            .map(item => item.author)
            .filter(author => author && author.trim() !== ''));
        // Count unique sources
        const uniqueSources = new Set(knowledgeBase.items
...
*Note: Content has been automatically cleaned and enhanced using Gemini AI to improve readability and formatting.*`;
     ...
     */
    async serializeWithCleaning(documents, teamId, userId, outputPath) {
        if (!this.contentCleaner) {
            throw new Error('Content cleaning not available. Ensure GEMINI_API_KEY is set.');
        }
        const result = await this.contentCleaner.cleanAndExportDocuments(documents, teamId, userId, outputPath);
        if (!result.success) {
...
//# sourceMappingURL=knowledge-serializer.js.map